...
bool iSingleFoil     = true;
bool iSimpleDetector = false;
...
std::string mWorld      = "Vacuum";
std::string mLargeTank  = "Lead";
std::string mBrickWall  = "Gold";
std::string mSheetWall  = "Copper";
std::string mSingleFoil = "Copper";
float pSingleFoil = 0.;
...
CustomGeo()
{
  ...
  /** set magnetic field **/
  if (gMC && magneticField > 0.) gMC->SetMagField(new TGeoUniformMagField(0., 0., magneticField));
  
  /** create materials **/
  CreateMaterials();
  
  /** create world **/
  auto world = gGeoManager->MakeBox("World", media[mWorld], 2., 2., 2.);
...
CreateMaterials()
{
  ...
  /** Copper **/
  density = 8.96;
  materials["Copper"] = new TGeoMaterial("Copper", table->FindElement("Cu"), density);
  media["Copper"] = new TGeoMedium("Copper", media.size(), materials["Copper"], param);
...
{
  /** create lead sheet **/
  auto foil = gGeoManager->MakeBox("Foil", media[name], 0.05, 50., 50.);
